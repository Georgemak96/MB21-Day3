...
```{r}
data_vac %>% count(Booster_Doses_18Plus_Vax_Pct) %>% ggplot(aes(Booster_Doses_18Plus_Vax_Pct)) +
  geom_histogram()
...
knn_model <- train(pct_trump ~ Men+ Women+White+Asian+Black+Hispanic+Pacific+ VotingAgeCitizen + Income+ IncomePerCap+ Poverty+ ChildPoverty+ Professional+ Service+ Office+ Construction+ Production+Employed+ PrivateWork+ PublicWork+ SelfEmployed+ FamilyWork+ Unemployment +TotalPop+Drive+ Carpool+ Transit+ Walk+ OtherTransp+ WorkAtHome+ MeanCommute, data= data_vac_train_processed, method = "knn", trControl = tr, tuneGrid = tune_grid)
...
plot(knn_model)
###knn performs better in terms of fitting on the training set only and svm performs better both at the variance and the bias
```
...
```{r}
tr = trainControl(method = "repeatedcv", number = 5, repeats = 3)
svm_model <- train(pct_trump ~ Men+ Women+White+Asian+Black+Hispanic+Pacific+ VotingAgeCitizen + Income+ IncomePerCap+ Poverty+ ChildPoverty+ Professional+ Service+ Office+ Construction+ Production+Employed+ PrivateWork+ PublicWork+ SelfEmployed+ FamilyWork+ Unemployment +TotalPop+Drive+ Carpool+ Transit+ Walk+ OtherTransp+ WorkAtHome+ MeanCommute, data= data_vac_train_processed, method="svmRadial", trainControl = tr)
...
data_vac_train_processed_y = data_vac_train_processed$pct_trump
...
data_vac_test_processed_y <-data_vac_test_processed$pct_trump
...
rmse_lasso_test <- sqrt(mean(( data_vac_test_processed_y- predictions_test)^2))
```
...
abline(v = log(lasso_reg$lambda.1se)) ##all were eliminatedd
```
...
rmse_ridge_test <- sqrt(mean(( data_vac_test_processed_y- predictions_test)^2))
coef(ridge_reg)
...
```{r}
cbind(coef(reg_model), coef(lasso_reg), coef(ridge_reg)) ###lasso has no variables, linear regression has them all, ridge regression has shrunken towards zero a lot of them.
###Final comparison
